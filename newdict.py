...
class BST:
    ...
    def findsubstring(self, substring, counter):
        return self.searchSubstring(self.root, substring, counter)
    def searchSubstring(self, cn, substring, counter):
        while counter > 0:
            ...
            if cn is None:
                return False
            elif substring in cn.word:
                myword.append(cn.word)
                return self.searchSubstring(cn.rc, substring, counter)
            elif substring < cn.word:
                return self.searchSubstring(cn.lc, substring, counter)
            else:
                return self.searchSubstring(cn.rc, substring, counter)
            counter=counter-1
        return myword
...
if __name__ == '__main__':
    ...
    with open('outputPS14.txt', 'w') as f:
        ...
        for s in subList:
            f.write('\n---------------------------------------------------\n---------------- Sub String:'+s+'------------------------\n')
            sb=Tree.findsubstring(s, counter)
            if sb:
                f.write(str(sb))
            else:
                f.write(str(l[1]) + '- not found')